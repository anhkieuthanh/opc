...
@app.command(name="serve-paperclip-bridge")
def serve_paperclip_bridge(
    port: int | None = typer.Option(None, "--port", "-p", help="Paperclip bridge server port"),
    host: str | None = typer.Option(None, "--host", "-H", help="Bind address"),
    timeout: float | None = typer.Option(None, "--timeout", "-t", help="Per-request timeout (seconds)"),
    bridge_token: str | None = typer.Option(
        None,
        "--bridge-token",
        envvar="PAPERCLIP_BRIDGE_TOKEN",
        help="Shared token required on Paperclip bridge requests.",
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show nanobot runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    """Start the dedicated Paperclip dispatch bridge endpoint."""
    try:
        from aiohttp import web  # noqa: F401
    except ImportError:
        console.print("[red]aiohttp is required. Install with: pip install 'nanobot-ai[api]'[/red]")
    ...
    if not token:
        console.print(
            "[red]Error:[/red] Provide --bridge-token or set PAPERCLIP_BRIDGE_TOKEN before "
            "starting the Paperclip bridge."
        )
        raise typer.Exit(1)
    ...
    model_name, preset_tag = _model_display(runtime_config)
    console.print(f"{__logo__} Starting Paperclip bridge server")
    console.print(f"  [cyan]Endpoint[/cyan] : http://{host}:{port}{PAPERCLIP_BRIDGE_PATH}")
    console.print(f"  [cyan]Auth[/cyan]     : {PAPERCLIP_BRIDGE_AUTH_HEADER}: <shared-token>")
    console.print(f"  [cyan]Model[/cyan]    : {model_name}{preset_tag}")
    console.print(f"  [cyan]Timeout[/cyan]  : {timeout}s")
    if host in {"0.0.0.0", "::"}:
        console.print(
            "[yellow]Warning:[/yellow] Bridge is bound to all interfaces. "
            "Only do this behind a trusted network boundary, firewall, or reverse proxy."
        )
    console.print()
    ...
    bridge_app.on_startup.append(on_startup)
    bridge_app.on_cleanup.append(on_cleanup)

    web.run_app(bridge_app, host=host, port=port, print=lambda msg: logger.info(msg))
